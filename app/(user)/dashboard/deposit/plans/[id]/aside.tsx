...
const InvestmentPlanCard: React.FC<InvestmentPlanCardProps> = ({ id }) => {
  ...
  const handleInvestClick = async () => {
    ...
    const price = investmentPlan.price || investmentPlan.minAmount;
    
    if (amount < price) {
      toast.error(`Minimum investment amount is $${price}`);
      return;
    }
    ...
    try {
      await createInvestmentMutation.mutateAsync({
        ...
        duration: investmentPlan.duration || investmentPlan.durationHours,
        profitPercent: investmentPlan.profitPercent || investmentPlan.roi
      });
      ...
      toast.error(error.message || "Failed to create investment. Please try again.");
    }
  };
  ...
  min={investmentPlan.price || investmentPlan.minAmount}
